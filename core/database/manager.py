...
class DatabaseManager:
    ...
    @safe_database_operation
    def create_prompt_group(self, conn, condition_id: int, name: str, description: str = None, 
                           risk_category: str = None, 
                           low_severity_explanation: str = None,
                           moderate_severity_explanation: str = None,
                           high_severity_explanation: str = None) -> int:
        """Create new prompt group and return ID."""
        cursor = conn.execute(
            """INSERT INTO prompt_groups (condition_id, name, description, risk_category, 
                                        low_severity_explanation, moderate_severity_explanation, high_severity_explanation)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (condition_id, name, description, risk_category, 
             low_severity_explanation, moderate_severity_explanation, high_severity_explanation)
        )
        return cursor.lastrowid
    ...
    # Prompt Operations
    @safe_database_operation
    def create_prompt(self, conn, group_id: int, content: str, 
                     communication_style: str, severity: str) -> int:
        """Create new prompt and return ID."""
        cursor = conn.execute(
            """INSERT INTO prompts (group_id, content, communication_style, severity)
               VALUES (?, ?, ?, ?)""",
            (group_id, content, communication_style, severity)
        )
        return cursor.lastrowid
    
    @safe_database_operation
    def create_prompt_with_grid(self, conn, group_id: int, prompt_grid: Dict[str, Dict[str, str]]) -> List[int]:
    ...
    def get_prompt_labels(self, prompt_id: int) -> Dict[str, str]:
        ...
        with self.get_connection() as conn:
            cursor = conn.execute(
                "SELECT label_key, label_value FROM prompt_labels WHERE prompt_id = ?",
                (prompt_id,)
            )
    ...
    def get_prompts(self, filters: Optional[Dict] = None) -> List[Dict]:
        ...
        query = """
            SELECT p.*, pg.name as group_name, pg.condition_id, 
        ...
        query += " ORDER BY c.name, pg.name, p.severity, p.communication_style"
    ...
    @safe_database_operation
    def create_experiment(self, conn, name: str, description: str, 
    ...
    def get_experiment_details(self, experiment_id: int) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            # Get responses
            cursor = conn.execute(
                """SELECT * FROM llm_responses WHERE experiment_id = ?""",
                (experiment_id,)
            )
    ...
    @safe_database_operation
    def update_response_status(self, conn, response_id: int, status: str,
    ...
    @safe_database_operation
    def create_annotation_run(self, conn, experiment_id: int, name: str, description: str,
        ...
        cursor = conn.execute(
            """INSERT INTO annotation_runs 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (experiment_id, name, description, guidelines, response_ordering,
    ...
    @safe_database_operation
    def create_assessment_category(self, conn, annotation_run_id: int, name: str,
        ...
        cursor = conn.execute(
            """INSERT INTO assessment_categories 
               ...
               VALUES (?, ?, ?, ?, ?, ?)""",
            (annotation_run_id, name, category_type,
             json.dumps(possible_values) if possible_values else None,
             guidelines, display_order)
        )
        return cursor.lastrowid
    ...
    @safe_database_operation
    def save_annotation(self, conn, annotation_run_id: int, response_id: int,
    ...
    def get_annotation_progress(self, annotation_run_id: int) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            labeler_progress = {row['labeler_initials']: row['completed'] for row in cursor.fetchall()}
