...
class AnnotationManager:
    ...
    def get_eligible_responses(self, annotation_run_id: int) -> List[Dict]:
        ...
        for response in experiment["responses"]:
            ...
            if annotation_run["filter_conditions"]:
                ...
                prompt = next(
                    (p for p in experiment["prompts"] if p["id"] == response["prompt_id"]), None
                )
    ...
    def get_labeler_progress(self, annotation_run_id: int, labeler_initials: str) -> Dict:
        ...
        if total_responses == 0:
            return {"total_responses": 0, "completed_responses": 0, "completion_percentage": 0}
    ...
    def get_annotation_for_response(
        self, annotation_run_id: int, response_id: int, category_id: int, labeler_initials: str
    ) -> Optional[Dict]:
