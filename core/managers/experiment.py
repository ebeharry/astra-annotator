...
from core.llm import ClaudeProvider, GeminiProvider, GrokProvider, OllamaProvider, OpenAIProvider
...
class ExperimentManager:
    ...
    def create_experiment(
        self, name: str, description: str, models: List[str], prompt_ids: List[int]
    ) -> int:
    ...
    def _get_provider(self, model_name: str, api_keys: Dict[str, str] = None) -> Any:
        ...
        if model_name.startswith("gpt"):
            return OpenAIProvider(api_key=api_keys.get("openai"), model=model_name)
        elif "claude" in model_name.lower():
            return ClaudeProvider(api_key=api_keys.get("anthropic"), model=model_name)
        elif "grok" in model_name.lower():
            return GrokProvider(api_key=api_keys.get("xai"), model=model_name)
        elif "gemini" in model_name.lower():
            return GeminiProvider(api_key=api_keys.get("google"), model=model_name)
    ...
    async def _process_single_response(
        self, response_record: Dict, api_keys: Dict[str, str] = None
    ) -> Dict:
        ...
        try:
            ...
            if result["status"] == "success":
                self.db_manager.update_response_status(
                    response_record["id"], "completed", response_text=result["response"]
                )
                return {
                ...
                self.db_manager.update_response_status(
                    response_record["id"], "failed", error_message=result["error_message"]
                )
                return {
                    "id": response_record["id"],
                    "status": "failed",
            ...
            self.db_manager.update_response_status(
                response_record["id"], "failed", error_message=str(e)
            )
            return {
                "id": response_record["id"],
                "status": "failed",
    ...
    async def run_experiment(
        self, experiment_id: int, api_keys: Dict[str, str] = None, max_concurrent: int = 5
    ) -> Dict:
        ...
        try:
            ...
            if not pending_responses:
                ...
                return {"status": "completed", "message": "No pending responses to process"}
    ...
    def get_experiment_progress(self, experiment_id: int) -> Dict:
        ...
        return {
            ...
            "failed": failed,
            "pending": pending,
            "completion_percentage": (completed / total_responses * 100)
