...
def show_labeler_login(active_runs: List[Dict], annotation_manager: AnnotationManager):
    ...
    with col1:
        ...
        labeler_initials = st.text_input(
            "Labeler Initials",
            placeholder="e.g., JD, MS, etc.",
            key="labeler_initials_input"
        )
...
def show_annotation_interface(annotation_manager: AnnotationManager, db: DatabaseManager):
    ...
    if run_details['response_ordering'] == 'randomized':
        ...
        # Sequential ordering
        ordered_responses = sorted(eligible_responses, 
    ...
    with col4:
        if st.button("🏠 Main Menu"):
            ...
            for key in ['labeler_initials', 'selected_annotation_run', 'current_response_index', 'randomized_response_order']:
    ...
    with db.get_connection() as conn:
        cursor = conn.execute("SELECT content FROM prompts WHERE id = ?", (current_response['prompt_id'],))
    ...
    with st.form(key="annotation_form"):
        ...
        for i, category in enumerate(run_details['assessment_categories']):
            ...
            existing_justification = existing_annotation['justification'] if existing_annotation else ""
            ...
            if category['type'] == 'categorical':
                value = st.radio(
                    ...
                    index=category['possible_values'].index(existing_value) if existing_value in category['possible_values'] else 0,
        ...
        if save_button or save_and_next:
            ...
            if missing_categories:
                st.error(f"Please complete these required categories: {', '.join(missing_categories)}")
            else:
                # Save annotations
                try:
