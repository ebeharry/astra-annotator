...
def show_create_annotation_run(annotation_manager: AnnotationManager, 
    ...
    with st.form("create_annotation_run_form"):
        ...
        if selected_experiment:
            ...
            with col1:
                st.metric("Total Responses", len([r for r in experiment_details['responses'] if r['status'] == 'completed']))
            with col2:
                st.metric("Models", len(experiment_details['models']))
            with col3:
                unique_conditions = set(p.get('condition_name') for p in experiment_details['prompts'])
        ...
        if selected_experiment:
            ...
            model_selection = st.radio(
                "Model Selection",
                ["Include All Models", "Select Specific Models"],
                horizontal=True
            )
        ...
        if selected_experiment:
            available_conditions = list(set(p.get('condition_name') for p in experiment_details['prompts']))
            selected_conditions = []
        ...
        response_ordering = st.radio(
            ...
            format_func=lambda x: "Sequential (by condition/group)" if x == "sequential" else "Randomized",
            horizontal=True
        )
            ...
            for i, category in enumerate(st.session_state.assessment_categories):
                with st.container():
                    ...
                    with col1:
                        ...
                        if category['type'] == 'categorical' and isinstance(category['possible_values'], list):
                            values_text = st.text_input(
                                "Possible Values (comma-separated)",
                                value=', '.join(category['possible_values']) if category['possible_values'] else '',
                                key=f"cat_values_{i}"
                            )
                            if values_text:
                                category['possible_values'] = [v.strip() for v in values_text.split(',') if v.strip()]
        ...
        if submitted:
            ...
            try:
                ...
                annotation_run_id = annotation_manager.create_annotation_run(
                    ...
                    filter_models=selected_models if model_selection == "Select Specific Models" else None,
                    filter_conditions=selected_conditions if condition_selection == "Select Specific Conditions" else None
                )
                ...
                for i, category in enumerate(st.session_state.assessment_categories):
                    annotation_manager.create_assessment_category(
                        ...
                        possible_values=category['possible_values'] if category['type'] != 'text' else None,
...
def show_preview_interface(annotation_manager: AnnotationManager, db: DatabaseManager):
    ...
    if selected_run:
        ...
        with db.get_connection() as conn:
            cursor = conn.execute("SELECT content FROM prompts WHERE id = ?", (sample_response['prompt_id'],))
        ...
        for category in run_details['assessment_categories']:
            ...
            st.text_area(
                "Justification (optional)",
                key=f"preview_just_{category['id']}",
                disabled=True
            )
