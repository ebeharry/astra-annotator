    ...
    for data in progress_data:
        ...
        with st.expander(f"📊 {run['name']} ({run['status']})"):
            ...
            with col1:
                ...
                st.write(f"**Overall Progress:** {data['overall_completion']:.1f}% ({data['total_completed']}/{data['total_expected']})")
...
def show_reliability_analysis(annotation_manager: AnnotationManager, annotation_runs: List[Dict], db: DatabaseManager):
...
def show_detailed_analysis(annotation_manager: AnnotationManager, annotation_runs: List[Dict], db: DatabaseManager):
    ...
    with col2:
        ...
        selected_labelers = st.multiselect(
            "Labelers",
            options=available_labelers,
            default=available_labelers
        )
    
    with col3:
        # Assessment category filter
        available_categories = [(cat['id'], cat['name']) for cat in run_details['assessment_categories']]
    ...
    summary_stats = filtered_df.groupby(['model', 'category_name', 'value']).size().reset_index(name='count')
    ...
    if not disagreement_responses.empty:
        ...
        disagreement_display = disagreement_responses[['response_id', 'model', 'category_name', 'disagreement_count', 'values']].head(10)
...
def show_export_results(annotation_manager: AnnotationManager, annotation_runs: List[Dict], db: DatabaseManager):
...
def add_response_metadata(annotations_df: pd.DataFrame, run_id: int, db: DatabaseManager) -> pd.DataFrame:
...
def calculate_categorical_agreement(reliability_matrix: pd.DataFrame) -> Dict:
    ...
    for idx, row in complete_cases.iterrows():
        values = row.values
        n_raters = len(values)
...
def generate_export_data(runs: List[Dict], annotation_manager: AnnotationManager, 
                        db: DatabaseManager, **options) -> pd.DataFrame:
    ...
    for run in runs:
        ...
        if not annotations_df.empty:
            ...
            columns_to_keep = [
                'annotation_run_name', 'annotation_run_id', 'experiment_name',
                'response_id', 'category_name', 'labeler_initials', 'value'
            ]
            ...
            if options.get('include_metadata', True):
                columns_to_keep.extend(['model', 'communication_style', 'severity', 'condition_name'])
            ...
            available_columns = [col for col in columns_to_keep if col in annotations_with_metadata.columns]
