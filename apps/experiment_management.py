...
def run_experiment_async(experiment_manager: ExperimentManager, exp_id: int, api_keys: Dict, max_concurrent: int):
...
def show_experiment_list(experiment_manager: ExperimentManager):
    ...
    if any(exp_id in st.session_state.running_experiments 
            ...
            with col1:
                ...
                if is_running:
                    if running_info.get('status') == 'completed':
                        ...
                        st.write(f"**Results:** {result.get('completed', 0)} completed, {result.get('failed', 0)} failed")
                    elif running_info.get('status') == 'failed':
                        st.error(f"❌ Background execution failed: {running_info.get('error', 'Unknown error')}")
                    else:
...
def show_create_experiment(experiment_manager: ExperimentManager, db: DatabaseManager):
    ...
    if not prompts:
        st.warning("⚠️ No prompts available. Please add prompts first in the Prompt Management section.")
        return
    ...
    with st.form("create_experiment_form"):
        ...
        selection_method = st.radio(
            "Selection Method",
            ["Select by Groups", "Select Individual Prompts", "Select All"]
        )
        ...
        if selection_method == "Select by Groups":
            ...
            if selected_groups:
                ...
                st.write(f"Selected {len(selected_prompt_ids)} prompts from {len(selected_groups)} groups")
        ...
        if submitted:
            ...
            try:
                ...
                st.success(f"✅ Experiment '{exp_name}' created successfully! (ID: {experiment_id})")
                st.write(f"• Models: {', '.join(selected_models)}")
                st.write(f"• Prompts: {len(selected_prompt_ids)}")
                st.write(f"• Total responses to generate: {len(selected_models) * len(selected_prompt_ids)}")
...
def show_run_experiments(experiment_manager: ExperimentManager):
    ...
    runnable_experiments = [
        ...
        if exp['status'] in ['created', 'failed']
    ]
    
    if not runnable_experiments:
        st.info("No experiments available to run. Create an experiment first or all experiments are already completed.")
        return
    ...
    if selected_exp:
        ...
        if is_running:
            if running_info.get('status') == 'completed':
                ...
                st.write("The page will auto-refresh to show progress. You can navigate to other pages while it runs.")
        ...
        with col2:
            ...
            max_concurrent = st.number_input("Max Concurrent Requests", min_value=1, max_value=20, value=5)
        ...
        if st.button("🚀 Run Experiment", type="primary"):
            ...
            st.info("The experiment will run in the background. You can navigate to other pages and check back for progress.")
